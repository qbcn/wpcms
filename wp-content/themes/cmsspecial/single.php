 ...
 <div class="title_page"><h1><?php the_title();?></h1></div><div class="des_page">
		 ...
		 $id =get_the_ID(); 
		  if(!get_post_meta($id,"bbs_shoppingbox", true)){?>
            <p class="infot"><em>发布时间：<?php echo get_the_time('Y/m/d') ; ?></em>
            <em> <?php   foreach((get_the_category()) as $category) { echo '<a href="'. get_category_link($category->cat_ID).'">' .$category->cat_name .'</a> ';} }?></em>
            <em><?php $posttags = get_the_tags(); if ($posttags) {echo '标签：'; foreach($posttags as $tag) { echo '<a target="_blank" id="tagss" href="'.get_bloginfo('url').'/tag/'.$tag->slug.'">' .$tag->name .'</a>';}}?></em><em>浏览：<?php echo $getPostViews; ?>  </em> </p>
