...
class pic_text extends WP_Widget {

	function pic_text()
	{
		$widget_ops = array('classname'=>'pic_text','description' => get_bloginfo('template_url').'/images/xuanxiang/6.gif');
		$control_ops = array('width' => 200, 'height' => 300);
		parent::WP_Widget($id_base="pic_text",$name='图文模块【02】',$widget_ops,$control_ops);  
	...
	function form($instance) { 
		 ...
		 $target = esc_attr($instance['target']);
		  ...
		  $pic_texts = esc_attr($instance['pic_texts']); 
		   $pic_texts_pic = esc_attr($instance['pic_texts_pic']);
		    $pic_texts_link = esc_attr($instance['pic_texts_link']);
...
<a target="_blank" href="http://www.themepark.com.cn/ffmhwordpressqyzt.html"> 查看付费版详情</a>
 ...
 <script language="javascript">(function(a){a(function(){a(".color-field_w").wpColorPicker();a(".customize-control-widget_form .widget-control-save").fadeIn()})})(jQuery);jQuery(document).ready(function(){var b;var a;jQuery(".upload_button_w").on("click",function(c){a=jQuery(this).prev("input");b=wp.media({title:"选择图片",button:{text:"选择"},multiple:false});if(b){b.open()}b.on("select",function(){attachment=b.state().get("selection").first().toJSON();jQuery(a).val(attachment.url);jQuery(".supports-drag-drop").remove()})})});
	...
	extract( $args );
	    ...
	    $pic_texts = apply_filters('widget_title', empty($instance['pic_texts']) ? __('') : $instance['pic_texts']);
		 $pic_texts_pic = apply_filters('widget_title', empty($instance['pic_texts']) ? __('') : $instance['pic_texts_pic']);
		...
		$nunber =apply_filters('widget_title', empty($instance['nunber']) ? __('4') : $instance['nunber']);
		 $timess=apply_filters('widget_title', empty($instance['timess']) ? __('7') : $instance['timess']);
 $pic_texts_link=apply_filters('widget_title', empty($instance['pic_texts_link']) ? __('7') : $instance['pic_texts_link']);
...
if( $zhiding =="2" ){ $oder="rand";}else{ $oder="ASC";}
 ...
 $query = new WP_Query( $args );      
	if(!$query->have_posts()) { get_template_part( 'index/pictext' ); }else{
		
		?>
<div class="pictext  box">
<div class="m_title"><?php if($title){?><a target="_blank" href="<?php echo get_category_link($cat) ?>"><b><?php echo $title ?></b><?php echo $title2 ?></a><?php } ?></div> 
 ...
 if($ashu_i==1){?>           
     ...
     <div class="pictext_pic">
     
     <a <?php echo $tagerts ?> title="<?php the_title(); ?>" href="<?php if($linkss !=""){echo $linkss;}else{echo get_permalink();}; ?>" class="pictext_pic_a">
     
      <?php  if (has_post_thumbnail()) { the_post_thumbnail('pic_r' ,array('alt'	=>$tit, 'title'=> $tit ));}
		   else{echo '<img src="'. get_bloginfo('template_url').'/images/demo/vedio.gif" />';} ?>
     </a>
      <b><a href="#"><?php  echo mb_strimwidth(strip_tags(apply_filters('the_title', $tit)),  0,50,"...",'utf8'); ?></a></b>
   ...
   <?php }else{ ?>
    ...
    <li class="first_v">
         <b> <a <?php echo $tagerts ?> title="<?php the_title(); ?>" href="<?php if($linkss !=""){echo $linkss;}else{echo get_permalink();}; ?>">
     <?php  echo mb_strimwidth(strip_tags(apply_filters('the_title', $tit)),  0,50,"...",'utf8'); ?>
     </a></b>
